...
- **4 Modules**:
  ...
  2. **Compliance Check** — runs the full house rule engine against every row. **v2** (per the manager's Sept 2026 "Updated Master Checklist"):
...
Mining all 10 historical change logs surfaced a small number of points where **different editions carry contradictory rulings**. Rather than silently pick a side, the app does **not** hard-code a rule for these — they need a decision from Syazwan/the director, then a one-line change to `compliance.ts`:

1. **Impact grade format — single word vs multi-word tier.** The **4 Sep 2026** change log states explicitly (item 24): *"Every Impact cell opens with one word from Positive, Negative, Mixed, Neutral, then a full stop... No qualifiers attached to the label... Syazwan has ruled."* This directly contradicts the **8-label tiered system** (adding Strategic Benchmark / Opportunity Watch / Policy Watch / High Strategic Relevance) from the **7 Sep 2026 PPTX checklist v2** that this app currently enforces as `IMPACT_OPENERS` in `compliance.ts`. The PPTX post-dates the ruling by 3 days, so it may supersede it — but this needs explicit confirmation, since the 4 Sep log frames it as a considered, named decision. **Action needed**: confirm whether the 8-tier system is the deliberate successor to the plain-4-grade ruling, or whether the PPTX checklist should be corrected back to 4 grades.
2. **Currency-code spacing.** The **11 Sep 2026** log (item 1) rules "RM170.5 bn" with **no space** between the currency code and the figure, closing the space at every occurrence. Earlier editions (e.g. 24 Jul #37, 27-31 Jul B1) consistently use "RM 1.5 bn" **with a space**. The most recent ruling (11 Sep) should normally win, but it directly reverses months of prior style — worth a one-line confirmation before the compliance engine bans one form in favour of the other.
3. **Regulatory cell when nothing changed — dash vs boilerplate sentence.** The **6 Aug 2026** review (item 3) rules that a blank Regulatory cell must read *"No new regulatory or policy changes were introduced."* instead of a bare "-". This app implements that as an **info-level** nudge only (`regulatory_boilerplate` rule), not an error, because no later edition explicitly re-confirms or reverses it, and a bare "-" continued to appear in later logs (e.g. 21 Aug) without being flagged again — it's unclear whether the 6 Aug ruling was adopted going forward or was a one-off note for that edition.
4. **Compound grade labels ("Mixed/Industry Read-Through", "Neutral/Competitive Benchmark").** The **7 Aug 2026** log documents these as a **third, interim** grading style sitting between the old 4-grade system and the new 8-label system, explicitly flagged as "the compound-label ruling still outstanding" pending a decision. This style is now superseded by the 8-label PPTX checklist and is **not** implemented — noted here only so it is not mistaken for a still-open format if it resurfaces.

None of the above are silently resolved in code — each is a deliberate omission pending your confirmation, per the "don't repeat the same mistakes" instruction not extending to "guess when the house itself disagreed with itself."
...
- **Last Updated**: 2026-09-24 (v3 compliance/redundancy rules added from historical change-log mining)
