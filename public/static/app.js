...
async function renderBriefDetail(content, briefId) {
  ...
  content.innerHTML = `
    ...
    <div class="flex gap-1 border-b mb-4">
      ...
      <a href="#/brief/${briefId}/fact-check" class="tab-btn ${activeTab === 'fact-check' ? 'active' : ''}"><i class="fas fa-magnifying-glass-chart mr-1"></i>Deep Fact Check</a>
    </div>
  ...
  } else if (activeTab === 'fact-check') {
    await renderFactCheckTab(tabContent, brief)
  }
}
...
// --- Deep Fact Check tab ------------------------------------------------
// Reports are NOT generated by a live in-app button — a Cloudflare Worker
// can't do open-ended live web search without a paid third-party search
// API (not yet set up). Instead the consultant asks their AI assistant
// (which has real multi-source web-search + verification tools) to run the
// check and post the structured result via POST .../fact-check-reports.
// This tab just lists/displays/deletes whatever's been generated that way.
async function renderFactCheckTab(el, brief) {
  el.innerHTML = `<div class="flex items-center justify-center py-10"><div class="spinner"></div></div>`
  const reports = await API.listFactCheckReports(brief.id)

  const howToBox = `
    <div class="card p-4 mb-4 text-sm text-slate-600 bg-teal-50 border-teal-200">
      <div class="font-semibold text-teal-900 mb-1"><i class="fas fa-circle-info mr-1"></i>How to run a Deep Fact Check</div>
      <p>This checks every claim in the brief against multiple <em>independently-named</em> external sources (Reuters, BNM, The Star, company press releases, etc.) — not just the brief's own embedded link — and produces a topic-grouped report with quoted verification text, exactly like a director-ready fact-check memo.</p>
      <p class="mt-1">A Cloudflare Worker can't do open-ended live web search on its own, so this isn't a one-click in-app button yet. Instead: ask your AI assistant to "run a Deep Fact Check on Brief #${brief.id}" — it will search, verify, and save the report here automatically.</p>
    </div>
  `

  if (reports.length === 0) {
    el.innerHTML = howToBox + emptyState('No Deep Fact-Check reports yet', 'Ask your AI assistant to run one for this brief — see instructions above.')
    return
  }

  el.innerHTML = howToBox + `
    <div class="space-y-3">
      ${reports.map((r) => `
        <div class="card p-4 cursor-pointer hover:border-teal-700 transition-colors" data-open-report="${r.id}">
          <div class="flex items-start justify-between gap-3">
            <div>
              <div class="font-semibold text-slate-800">${escapeHtml(r.title)}</div>
              <div class="text-sm text-teal-800 font-medium mt-0.5">${escapeHtml(r.overall_verdict)}</div>
              <div class="text-xs text-slate-400 mt-1">${r.claim_count} claims checked · ${r.confirmed_count} confirmed · ${r.discrepancy_count} flagged · generated ${escapeHtml(r.created_at)}</div>
            </div>
            <button class="text-slate-400 hover:text-red-600" data-delete-report="${r.id}" title="Delete"><i class="fas fa-trash"></i></button>
          </div>
        </div>
      `).join('')}
    </div>
  ...
  el.querySelectorAll('[data-open-report]').forEach((card) => {
    card.addEventListener('click', async (e) => {
      if (e.target.closest('[data-delete-report]')) return
      const reportId = card.dataset.openReport
      await showFactCheckReportModal(brief.id, reportId)
    })
  })
  el.querySelectorAll('[data-delete-report]').forEach((btn) => {
    btn.addEventListener('click', async (e) => {
      e.stopPropagation()
      if (!confirm('Delete this fact-check report?')) return
      await API.deleteFactCheckReport(brief.id, btn.dataset.deleteReport)
      toast('Report deleted')
      renderFactCheckTab(el, brief)
    })
  })
}

async function showFactCheckReportModal(briefId, reportId) {
  openModal(`<div class="p-8 flex items-center justify-center"><div class="spinner"></div></div>`)
  const report = await API.getFactCheckReport(briefId, reportId)

  const markerBadge = (marker) => marker === '✅'
    ? `<span class="text-emerald-600">✅</span>`
    : `<span class="text-amber-600">⚠️</span>`

  const claimHtml = (claim) => `
    <div class="border border-slate-200 rounded-lg p-3 mb-2.5">
      <div class="flex items-start gap-2">
        ${markerBadge(claim.marker)}
        <div class="flex-1">
          <div class="text-sm font-semibold text-slate-500">Claim ${claim.claim_number}</div>
          <div class="text-sm text-slate-800 italic mt-0.5">"${escapeHtml(claim.quoted_text)}"</div>
          <div class="mt-2 space-y-1.5">
            ${(claim.sources || []).map((s) => `
              <div class="text-xs bg-slate-50 border border-slate-200 rounded px-2.5 py-1.5">
                <div class="text-slate-600">"${escapeHtml(s.quote)}"</div>
                <div class="text-slate-400 mt-0.5">
                  — ${escapeHtml(s.source_name)}${s.source_date ? `, ${escapeHtml(s.source_date)}` : ''}
                  ${s.source_url ? ` · <a href="${escapeHtml(s.source_url)}" target="_blank" class="text-teal-700 hover:underline"><i class="fas fa-arrow-up-right-from-square"></i> link</a>` : ''}
                </div>
              </div>
            `).join('')}
          </div>
          <div class="mt-1.5"><span class="badge ${claim.marker === '✅' ? 'badge-ok' : 'badge-warning'}">${escapeHtml(claim.verdict_tag)}</span></div>
        </div>
      </div>
    </div>
  `
  ...
  const sectionHtml = (section) => `
    <div class="mb-5">
      <div class="font-bold text-teal-900 text-sm mb-2 pb-1 border-b border-teal-100">${escapeHtml(section.section_name)}</div>
      ${(section.claims || []).map(claimHtml).join('')}
    </div>
  `
  ...
  openModal(`
    <div class="p-5 border-b flex items-center justify-between">
      <div>
        <h3 class="font-bold text-teal-900">${escapeHtml(report.title)}</h3>
        <div class="text-sm text-teal-700 font-medium mt-0.5">${escapeHtml(report.overall_verdict)}</div>
      </div>
      <button class="text-slate-400 hover:text-slate-700" onclick="closeModal()"><i class="fas fa-xmark text-lg"></i></button>
    </div>
    <div class="p-5 max-h-[70vh] overflow-y-auto">
      <p class="text-sm text-slate-600 mb-5">${escapeHtml(report.overall_summary)}</p>

      ${(report.sections || []).map(sectionHtml).join('')}

      <div class="mt-5">
        <div class="font-bold text-teal-900 text-sm mb-2">Summary Table</div>
        <div class="card overflow-hidden">
          <table class="data-table">
            <thead><tr><th style="width:12%">Claim #</th><th>Verdict</th></tr></thead>
            <tbody>
              ${(report.summary_table || []).map((row) => `
                <tr><td class="font-semibold">${row.claim_number}</td><td class="text-slate-600">${escapeHtml(row.verdict_line)}</td></tr>
              `).join('')}
            </tbody>
          </table>
        </div>
      ...
      <div class="mt-5">
        <div class="font-bold text-teal-900 text-sm mb-1.5">Conclusion</div>
        <p class="text-sm text-slate-600">${escapeHtml(report.conclusion)}</p>
      </div>

      ${(report.minor_issues || []).length ? `
        <div class="mt-4">
          <div class="font-bold text-teal-900 text-sm mb-1.5">Minor issues found</div>
          <ul class="text-sm text-slate-600 list-disc list-inside space-y-0.5">
            ${report.minor_issues.map((i) => `<li>${escapeHtml(i)}</li>`).join('')}
          </ul>
        </div>
      ` : ''}

      <div class="text-xs text-slate-400 mt-5 pt-3 border-t">Generated ${escapeHtml(report.created_at)} · ${report.claim_count} claims checked</div>
    </div>
    <div class="p-5 border-t flex justify-end gap-2">
      <button class="btn btn-secondary" id="btn-copy-report"><i class="fas fa-copy"></i> Copy as text (for director email)</button>
      <button class="btn btn-primary" onclick="closeModal()">Close</button>
    </div>
  `)

  document.getElementById('btn-copy-report').addEventListener('click', () => {
    navigator.clipboard.writeText(report.report_markdown).then(() => toast('Copied to clipboard'))
  })
}
